...
READING = """  <h2>Reading this against a frozen lake</h2>
  ...
  Their spectrograms match typical corvid calls. They carry energy well above 2
  kHz. They sit 3–11 dB above everything else. The other nine species are all
...
{band_table}
  ...
  Local guides call that sound <em>long hou</em>, “dragon roar”. A likely
  explanation is that BirdNET maps ice onto birds whose calls are booms.
  <a href="https://xeno-canto.org/species/Botaurus-stellaris">Great Bittern</a> at
  0.906 is the strongest false positive. Its twelve detections arrive in sustained runs.
  ...
  to the dragon roar. Listen to them here, then listen to any green stretch of
  the timeline above:</p>
{reference_players}
  <p class="report-explainer">In listening tests, the ice carries an electronic
  quality that the bird does not. That difference is audible. None of the
  ...
  calls it Ruddy Shelduck at 0.928. That is the second highest confidence in the
  whole run. It is Xiao Zhang, one of the guides. He was calling across the lake
  to the recordist. The recordist identified it on listening. There is no bird
  in those three seconds at all.</p>
  <p class="report-explainer">Open that frame when reviewing the report. It also
  illustrates how a spectrogram can be misread. It carries harmonic stacks between
  1 and 3 kHz. We first took them for a call. Ice does not produce structure like
  that. We moved the species out of the doubtful list. A voice does produce it.
  The stacks are a pitch contour gliding through its harmonics. Formants sit
  around 1.2 and 2 kHz. Four or five syllables appear in the last second. That is
  speech, not song. The band average in the table above had hidden the voice under
  the low-frequency bed. The picture that corrected the first reading was then
  read incorrectly. Listening was required to resolve it.</p>
  ...
  source WAV before citation. The frame at 01:41:05 is the main reason: the
