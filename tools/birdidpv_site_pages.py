...
PHOTO_NOTE = """  <p class="report-explainer">Reference photographs come from
  ...
  Nine of these eleven rows are flagged for manual review. One of those nine is a
...
READING = """  <h2>Reading this against a frozen lake</h2>
  <p class="report-explainer">Two corvids make up 99 of the 128 detections, and
  both match the audio clearly. They carry energy well above 2 kHz, and they sit
  3–11 dB above everything else. The other nine species share 29 frames, and all
  of them are deep-voiced waterbirds. The lake had frozen over by mid-January at
  4,055 m, and we measured the detected frames after a 150 Hz high-pass, so that
  measurement shows where the two groups part:</p>
{band_table}
  <p class="report-explainer">The waterbird detections concentrate their energy
  below 400 Hz at a lower level, and they mostly show none of the high-frequency
  structure the corvid detections do. That band is where the ice resonance sits,
  and local guides call that sound <em>long hou</em>, “dragon roar”. BirdNET
  often maps ice onto birds whose calls are booms, and
  <a href="https://xeno-canto.org/species/Botaurus-stellaris">Great Bittern</a> at
  0.906 is the strongest false positive. Its twelve detections arrive in sustained
  runs, and eight of them fall inside seventy seconds, but not one of their
  spectrograms shows a call above the ice.</p>
  <p class="report-explainer">Compare the recordings to understand the mistake.
  On the first page of xeno-canto recordings for
  ...
  <a href="https://xeno-canto.org/741523">XC741523</a>. The booms sound similar
  to the dragon roar, so listen to them here, and then listen to any green stretch
  of the timeline above:</p>
{reference_players}
  <p class="report-explainer">In listening tests, the ice carries an electronic
  quality that the bird does not, and that difference is audible, but none of the
  spectrogram statistics on this page captured it.</p>
  <p class="report-explainer">The frame at 01:41:05 is also not a bird. BirdNET
  calls it Ruddy Shelduck at 0.928, which is the second highest confidence in the
  whole run, but there is no bird in it. It is Xiao Zhang, one of the guides,
  calling across the lake to the recordist, who identified it on listening.</p>
  <p class="report-explainer">Open that frame when reviewing the report, because
  it also shows how a spectrogram can be misread. It carries harmonic stacks
  between 1 and 3 kHz, and we first took them for a call, but ice does not
  produce structure like that. We moved the species out of the doubtful list,
  and a voice does produce it: a pitch contour gliding through its harmonics,
  formants around 1.2 and 2 kHz, four or five syllables in the last second. That
  is speech, not song. The band average in the table above had hidden the voice
  under the low-frequency bed, and the picture that corrected the first reading
  was then read incorrectly, so listening was required to resolve it.</p>
  <p class="report-explainer">The detections fall into three groups: confirmed
  calls, lake ice, and at least one human voice. Every row marked
  <span class="flagged-note">check by ear</span> should be checked against the
  source WAV before citation, and the frame at 01:41:05 is the main reason: the
  model's second most confident bird in three hours was a person calling across
  the lake.</p>"""
